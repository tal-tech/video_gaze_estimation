...
class FaceModel:
    ...
    def estimate_head_pose(self, face: Face, camera: Camera) -> None:
        ...
        _, rvec, tvec = cv2.solvePnP(self.LANDMARKS, # 相机坐标系中的3D坐标点，单位m
                                     face.landmarks, # 图像坐标系中的2D坐标点
                                     camera.camera_matrix, # 相机内参矩阵 （需要通过标定得到）
                                     camera.dist_coefficients, # 畸变系数 （需要通过标定得到）
                                     rvec, # 旋转向量
                                     tvec, # 平移向量
                                     useExtrinsicGuess=True, # 是否输出旋转和平移矩阵
                                     flags=cv2.SOLVEPNP_ITERATIVE)
        # print('rvec:', rvec, '\n', 'tvec:', tvec)
        face.head_pose = (rvec, tvec)
